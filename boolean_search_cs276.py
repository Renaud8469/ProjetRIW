...
try:
    index_cs276 = open('static/index_cs276.json', 'r')
    docs_cs276 = open('static/docs_cs276.txt', 'r')
    vocabulary_cs276 = open('static/voc_cs276.json', 'r')
    print("Index détecté dans le dossier static/, chargement des fichiers détectés en cours...")
    index = json.load(index_cs276)
    vocabulary = json.load(vocabulary_cs276)
    docs = json.load(docs_cs276)
    print("Index chargé !\n")
